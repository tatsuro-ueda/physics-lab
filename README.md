...
**⚠️ リポジトリ直下の `acceleration.html` / `gyroscope.html` / `magnetometer.html` は生成物。直接編集しない。**
編集するのは `src/` の方。編集したら `python3 build.py` を実行すると、
`src/lab-common.js` を各ページに埋め込んだ**単一HTML**が直下に生成される（依存はPython標準のみ）。
配信されるのはこの単一HTMLなので、⬇や「名前を付けて保存」でそのまま配布できる。
