...
// ---- graph tap hit testing ----
const GRAPH_LINE_HIT_PX = 44;
const GRAPH_CLOSE_DISTANCE_PX = 80;

function pointToSegmentDistance(px, py, start, end) {
  const dx = end.x - start.x;
  const dy = end.y - start.y;
  const lengthSquared = dx * dx + dy * dy;
  if (lengthSquared === 0) return Math.hypot(px - start.x, py - start.y);
  const ratio = Math.max(0, Math.min(1, ((px - start.x) * dx + (py - start.y) * dy) / lengthSquared));
  return Math.hypot(px - (start.x + ratio * dx), py - (start.y + ratio * dy));
}

function classifyGraphTap(tapX, tapY, points) {
  ...
  for (const point of points) {
    ...
    const pointDistance = Math.hypot(tapX - point.x, tapY - point.y);
    if (pointDistance < distance) {
      distance = pointDistance;
      index = point.idx;
    }
    if (previous) {
      const segmentDistance = pointToSegmentDistance(tapX, tapY, previous, point);
      if (segmentDistance < distance) {
        distance = segmentDistance;
        index = Math.abs(tapX - previous.x) <= Math.abs(tapX - point.x) ? previous.idx : point.idx;
      }
    }
    previous = point;
  }

  if (distance <= GRAPH_LINE_HIT_PX) return { action: 'read', index, distance };
  if (distance <= GRAPH_CLOSE_DISTANCE_PX) return { action: 'ignore', index: null, distance };
  return { action: 'close', index: null, distance };
}
...
function createTimeSeriesCharts(cfg) {
  ...
  function attachInteractions(el, key, card) {
    ...
    el.addEventListener('pointermove', (e) => {
      ...
      if (cache.size === 2) {
        ...
        if (prevDist > 5 && dist > 5) {
          ...
          if (changed && cfg.onPinch) cfg.onPinch({ key });
        }
        prevDist = dist;
      } else if (cache.size === 1) {
        const zoomed = document.body.classList.contains('zoom');
        ...
        const yScale = plot.scales.y;
        if (Number.isFinite(yScale.min) && Number.isFinite(yScale.max) && yScale.max > yScale.min) {
          const dv = dy / rect.height * (yScale.max - yScale.min);
          viewV[key].autoScale = false;
          viewV[key].min = yScale.min + dv;
          viewV[key].max = yScale.max + dv;
          plot.setScale('y', { min: viewV[key].min, max: viewV[key].max });
        }
        if (cfg.onPan) cfg.onPan({ key });
      }
    });
    ...
    const endPointer = (e) => {
      ...
      if (cache.size === 1) {
        const remaining = Array.from(cache.values())[0];
        panPrevX = remaining.clientX;
        panPrevY = remaining.clientY;
      }
      if (tapStart && performance.now() - tapStart.t < 300 && cache.size === 0) {
        ...
        if (!zoomed) {
          ...
          const plot = uplots[key];
          let tapResult = { action: 'close', index: null };
          if (plot && series[key].t.length) {
            ...
            const points = (function* () {
              for (let idx = 0; idx < series[key].t.length; idx++) {
                const value = series[key].v[idx];
                if (value == null) {
                  yield null;
                  continue;
                }
                const x = plot.valToPos(series[key].t[idx], 'x');
                const y = plot.valToPos(value, 'y');
                yield Number.isFinite(x) && Number.isFinite(y) ? { x, y, idx } : null;
              }
            })();
            tapResult = classifyGraphTap(tapX, tapY, points);
            if (tapResult.action === 'read' && canReadValue()) {
              const idx = tapResult.index;
              const value = series[key].v[idx];
