...
function createXYZLab(cfg) {
  ...
  const AXES = ['x', 'y', 'z'];
  const COLORS = { x: '#ff9500', y: '#d4c400', z: '#e04040', abs: '#eeeeee' };
  ...
  function injectStyleOnce() {
    ...
    st.textContent =
      ...
      '.uplot-mount{width:100%;height:150px;background:#000;position:relative;touch-action:none}' +
  ...
  function setupChart(key) {
    ...
    const opts = {
      ...
      axes: [
        {
          stroke: '#888', ticks: { stroke: '#333', size: 4 }, grid: { stroke: '#2a2a2a' },
          font: '12px sans-serif',
          values: (u, ticks) => ticks.map(v => v.toFixed(v < 1 ? 1 : 0) + 's'),
        },
        {
          stroke: '#888', ticks: { stroke: '#333', size: 4 }, grid: { stroke: '#2a2a2a' },
          font: '12px sans-serif', size: 46,
        },
      ],
      series: [
        {},
