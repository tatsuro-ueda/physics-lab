...
# 3) 検証環境へ配置（VM上で。配るのはHTMLだけ）
sudo cp ~/physics-lab/*.html /srv/www/physics-lab/
```
...
- VM：`exedev@newtons-challenge.exe.xyz` の `~/physics-lab`（ローカルブランチは `master`）
