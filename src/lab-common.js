...
function createTimeSeriesCharts(cfg) {
  ...
  function setupChart(key) {
    ...
    mount.id = 'cv-' + key;
    mount.style.height = oldCanvas && oldCanvas.style.height ? oldCanvas.style.height : defaultHeight;
    ...
    if (cfg.enableHeadToggle) {
      const head = card.querySelector('.head');
      if (head) {
        head.addEventListener('click', () => {
          const zoomed = toggleCardZoom(card);
          clearMarkers();
          refreshVisible();
          if (cfg.onZoomChange) cfg.onZoomChange({ key, zoomed });
        });
      }
    }
